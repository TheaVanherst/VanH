...
let brightModeCS=[ //these colours use the RootLoop var, // BRIGHTMODE
        ...
        "#ff4fc7","#737475","#ff8ef4","#fd963b", // accent 5, inv, link and titles
    ...
    nightModeCS=[ //these colours use the RootLoop var. / DEMONMODE
        "#ce0547","#5a0bdf","#fb2b7f","#ffb500", //accent 1 through 4
        "#00a2ff","#100121","#fc8f57","#00eaff", // accent 5, inv, link and titles
        "#b4b4b4","#1f0229","#0d011f","#630667", //text, background, posts, art post back
        "#250042","#250042", //borders between posts, colour behind posts (these should really be the same tbh)
        "0.4","1","0"], //default opacity, fade to, logo invert var

    rootLoop=[ //root var to access while refering to colour schemes.
        "accent1","accent2","accent3","accent4",
        "alttitles","invaccent","links","titles",
...
var mobileBool = false, supportsTouch = false;
$(function(){
    if(/Mobi|Android/i.test(navigator.userAgent)){
        mobileBool = !mobileBool //sets mobile bool to true if a mobile OS is detected
...
function nightmode(){
    ...
    for(var b = 0; b < nightBoolArr.length; b++){
        $("#nm" + b + ", #twitter-widget-" + b).css({ //this sets the nightmode icon on the button to switch between night & daymodes.
    ...
    $('#header').css({"background-image":"url('/Website Assets/banner" + (nightBool ? 1:0) + ".png')"})
    docelem.style.setProperty('--profilepicture', "url('/Website Assets/pfp" + (nightBool ? 1:0) + ".gif')")
...
document.addEventListener("DOMContentLoaded", function(){
    ...
    if (rawCookieArr.find(a => a.includes('lightmode')) !== undefined) { //if cookie contains mode type
        ...
        nightBool = true} //else, just default to bright mode.
        ...
        setTimeout(function () {
            for (var i = 0; i < nightBoolArr.length; i++) { //check through night bool to figure which should be displayed.
                $("#twitter-widget-" + i).css( //addresses twitter widgets
                    "display",blockMode[nightBoolArr[i]])}

            if($("#twitter-widget-" + (!nightBool ? 0:1)).is(":visible")){check = true}
            else {loop()}
        }, 50);
    }());
})
